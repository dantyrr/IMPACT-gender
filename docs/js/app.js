...
class IMPACTApp {
    ...
    _renderInfluenceChart(journalData, seedPapers, seedsInJournal, localCyMap, citingPapers, totalCitedBy, papersData) {
        ...
        // --- New Monthly Citation Count Chart with same styling as main chart ---
        const newMonthlyContainer = document.getElementById('influence-monthly-new-container');
        if (seedsInJournal.length > 0 && allPapers.length > 0) {
            ...
            if (hasMonthlyData) {
                ...
                let monthlyDatasets;
                if (viewMode === 'individual' && seedsInJournal.length >= 1) {
                    // Individual view: same structure as main chart
                    const monthlyCensoredDataset = {
                        label: 'Individual PMID borders',
                        data: monthlyTotal.map(() => 0), // Baseline at 0
                        borderColor: chartManager.palette[7],
                        backgroundColor: 'transparent',
                        borderWidth: 1.5,
                        tension: 0.3,
                    ...
                    // Build inverted layers from total
                    ...
                    const monthlyLayerDatasets = seedsInJournal.map((seed, idx) => {
                        const palIdx = (idx + 2) % chartManager.palette.length;
                        const color = chartManager.palette[palIdx];
                        const r = parseInt(color.slice(1, 3), 16);
                        const g = parseInt(color.slice(3, 5), 16);
                    ...
                    monthlyDatasets = [monthlyOriginalDataset, monthlyCensoredDataset, ...monthlyLayerDatasets];
                } else {
                    // Combined view: simple stacked chart
                    const monthlyLayerDatasets = seedsInJournal.map((seed, idx) => {
                        const palIdx = (idx + 2) % chartManager.palette.length;
                        const color = chartManager.palette[palIdx];
                    ...
                    monthlyDatasets = [monthlyOriginalDataset, ...monthlyLayerDatasets];
                }

                chartManager._destroy('influence-monthly-new-chart');
                const ctxNew = document.getElementById('influence-monthly-new-chart');
                chartManager.charts['influence-monthly-new-chart'] = new Chart(ctxNew, {
                    type: 'line',
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            legend: { 
                                position: 'bottom',
                                onClick: null  // Disable legend clicks
                            },
                            tooltip: {
                                callbacks: {
                                    label: (ctx) => {
                                        if (viewMode === 'individual' && ctx.datasetIndex >= 2) {
                                            ...
                                            const prevVal = prevDs ? prevDs.data[ctx.dataIndex] : 0;
                                            const contrib = ctx.parsed.y - prevVal;
                                            return `${ctx.dataset.label}: ${Math.round(contrib).toLocaleString()} citations`;
                                        }
                                        return `${ctx.dataset.label}: ${Math.round(ctx.parsed.y).toLocaleString()} citations`;
                                    },
                                },
                            },
                        },
                ...
                newMonthlyContainer.style.display = 'none';
                chartManager._destroy('influence-monthly-new-chart');
            }
        } else {
            newMonthlyContainer.style.display = 'none';
